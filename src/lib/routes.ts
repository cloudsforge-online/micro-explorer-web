 ...
 *
 *   * a run of digits is a height — `indexer/src/server.ts:518` accepts `/^\d{1,15}$/` and answers
 *     **400 `bad_height`** for anything else;
 *   * `0x` + 64 hex is a hash — `EVM_HASH` at `indexer/src/server.ts:591`;
 *   * `0x` + 40 hex is an address — `EVM_ADDRESS` at `indexer/src/server.ts:590`.
 *
 * **Case is preserved.** The service lower-cases an EVM address itself
 * (`indexer/src/server.ts:601-608`) precisely so the EIP-55 checksum form every wallet and every
 ...
 * case-significant (`indexer/src/server.ts:610-616`). So the paste goes to the service as typed.
 *
