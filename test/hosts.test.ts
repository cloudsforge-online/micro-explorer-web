 ...
 * binds **4008** (`indexer/src/env.ts:363`, `indexer/.env.example:9`, `indexer/Dockerfile:91`).
 * micro-ui corrected the registry to 4008 (`ui/packages/ui/src/surfaces.ts:709`) and these pins
...
describe('the dev port disagreement, recorded rather than papered over', () => {
  ...
  it('the registry now gives explorer the port the indexer binds', () => {
    ...
    // micro-ui to 4008, which is what indexer/src/env.ts:363 binds, and the pin flipped with it.
