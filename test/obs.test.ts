...
describe('the envelope', () => {
  it('stamps the page the event came from', () => {
    ...
    assert.equal(wrapped.attributes['url'], 'https://cloudsforge.online/explorer/chains/ember/testnet')
    assert.equal(wrapped.route, '/explorer/chains/ember/testnet',
      // THE PUBLIC PATH. `route` is the column Lantern groups on, and every consolidated surface
      // now shares ONE origin — so a bare `/chains/…` from here would land in the same bucket as
      // any other surface's identically-named route and be indistinguishable in the chart.
      'the route is the column Lantern groups on')
    assert.equal(
